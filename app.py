...
SEC_USER_AGENT = os.getenv(
    "SEC_USER_AGENT",
    "SEC Filing Risk Lab/7.0 research app contact@example.com",
)
...
SCORING_VERSION = "v7.0-rule-based"
...
def filing_signals(text: str):
    ...
    for category, label, terms, severity in RULES:
        found = next((term for term in terms if term in lower), None)
        if not found:
            continue

        index = lower.find(found)
        snippet = text[max(0, index - 280): index + 560]
        hits.append(
            {
                "Category": category,
                "Signal": label,
...
def config_value(name: str, default: str = "") -> str:
    """Read deployment configuration from env first, then Streamlit secrets."""
...
with tab4:
    ...
    st.write(
        "The filing scanner searches for a small, transparent dictionary of review phrases (for example, material weakness, "
        "restatement, going-concern, covenant, impairment, and restructuring language). These signals are displayed separately "
        "because a keyword alone does not establish distress or wrongdoing."
    )
